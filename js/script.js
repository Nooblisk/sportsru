...
$("table").on("click", ".th-sortable", function(){
    ...
    var lastArray = mySort(teams, type);
    if (type != lastClick){
        lastClick = type;
        renderTable(lastArray);
    }else {
        renderTable(lastArray.reverse());
        lastClick = "";
    }
});
...
var templateRow = Handlebars.compile($('#templateRow').html());
renderTable(mySort(teams, "place"));

function mySort(array, element){
    function compare(a, b) {
        var aa = a[element];
        var bb = b[element];
        if (typeof(aa)!= "string")return aa - bb;
        else {
...
function renderTable(array){
    ...
    for (var i=0; i<array.length; i++){
        $("#bodyTable").append(templateRow(array[i]));
    };

    $(".flag").addClass(function(){
        var data = $(this).attr("title");
        return "flag-"+objFlags[data];
    })
}